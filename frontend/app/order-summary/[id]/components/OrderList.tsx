...
const OrderList = (props: Props) => {
    ...
    useEffect(()=>{
        const newSubtotal = orderList.reduce((acc, curr)=>
  ...
  return (
    <div
    className="md:w-[70%] max-w-225 mx-auto">
        <h2
        className="font-bold text-2xl text-center">Order Summary</h2>
        <div
        className="p-6">
            {orderList.map(item=>(
                <div
                ...
                className="flex md:gap-20 gap-6 py-6 border-b border-[rgba(0,143,171,0.3)] ">
                    ...
                    <div
                    className="flex flex-col gap-20 h-full my-auto">
                        <h2>{item.name}</h2>
        ...
        className="px-6">
                ...
                className="flex justify-between py-2">
                    <div>Promo Code</div>
                    <div
                    className="text-[#008FAB]">- $ {discount}</div>
                </div>
            </div>

            <div
            className="py-6 font-bold flex justify-between">
                <div>Total</div>
                <div>$ {total}</div>
            </div>
        </div>
